...
void get_password(char *storepassword)
{
   
    for(int i = 0 ; i < 5;i++)
    {         
        storepassword[i]=ext_eeprom_24C02_read(i);
    }
}

char login(char key ,char reset_flag)
{
    char storepassword[5];
    ...
    if(i == 4)
    {
        get_password(storepassword);
        if(strcmp(npassword,storepassword) == 0)// if password compare correct then return Login success   
        {        
